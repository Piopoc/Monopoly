...
class Table{
    public:
        Table(); // costruttore
        void print_matrix() const; // stampa il tabellone tramite il caricamento degli elementi in una "matrice"
        ...
        bool beyond_start(Player* p, int from) const; // verifica se player passa per casella con P
        void move(Player* p, int spostamenti); // spostamento del player da posizione attuale a posizione indicata da lancio dei dadi
        std::string get_cellname(int pos) const; // ritorna casella stile scacchiera o campo di battaglia navale
    ...
    private:
        ...
        int parametrizzazione_bordo_y(int t) const; // la componente y della curva che parametrizza il bordo
};
